...
int main(int argc, char* argv[])
{
    ...
    if ((
            find_field(Test->maxscales->conn_rwsplit[0], sel1,
                       "last_insert_id()", &last_insert_id1[0])
            != 0 ) || (
            find_field(Test->maxscales->conn_rwsplit[0], sel2,
    ...
    {
        Test->set_timeout(50);
        Test->add_result(execute_query(Test->maxscales->conn_rwsplit[0], "insert into t2 (x) values (%d);",
                                       i),
                         "Query failed");
