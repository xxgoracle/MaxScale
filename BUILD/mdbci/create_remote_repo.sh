...
if [ $1 == "full_repo" ] ; then
         find  -name "*.rpm" -path './maxscale-${major_ver}.*-release/mariadb-maxscale/${platform}/${platform_version}/*' -exec scp $scpopt {} $sshuser@$IP:src/ \;
         find  -name "*.deb" -path './maxscale-${major_ver}.*-release/mariadb-maxscale/${platform}/${platform_version}/*' -exec scp $scpopt {} $sshuser@$IP:src/ \;
else
