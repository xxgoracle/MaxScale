...
{
    // Assume that neither source mapping has duplicates. This allows to optimize the common case of
    // updating an empty target map.
    auto update_mapping = [](StringSetMap& target, StringSetMap&& source) {
        ...
        else
        {
            ...
            for (auto& source_elem : source)
            {
                ...
                {
                    // If the username does not yet exists, simply assign the contents.
                ...
                else
                {
                    ...
                    std::set_union(existing_elems.begin(), existing_elems.end(),
                                   ...
                                   std::inserter(sum, sum.begin()));
                    target[userhost] = std::move(sum);
                }
            }
        }
    };
