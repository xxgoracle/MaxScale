...
That is, the next time someone executes the statement `SELECT * FROM tbl` the
...
#### Invalidation
`storage_redis` supports invalidation, but the caveats documented [here](#invalidation)
are of greater significance as also the communication between the cache and the
cache storage is asynchronous and over the network.
